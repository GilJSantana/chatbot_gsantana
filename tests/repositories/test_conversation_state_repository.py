...
def test_get_by_session_id_not_found(db_session: Session, state_repository: ConversationStateRepository):
    """
...
def test_save_or_update_creates_new_state(db_session: Session, state_repository: ConversationStateRepository):
    """
    ...
    saved_state = db_session.query(ConversationState).filter(ConversationState.session_id == session_id).first()
...
def test_save_or_update_updates_existing_state(db_session: Session, state_repository: ConversationStateRepository):
    """
    ...
    updated_state = db_session.query(ConversationState).filter(ConversationState.session_id == session_id).first()

    assert updated_state is not None
    assert updated_state.state == new_state_name
    assert updated_state.data == new_data
    assert updated_state.data["nome"] == "Carlos" # Garante que os dados antigos foram mantidos/atualizados

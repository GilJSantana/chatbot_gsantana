...
from chatbot_gsantana.models.conversation_state import (
    ConversationState as ConversationStateModel,
)
...
def test_fsm_full_onboarding_flow_and_delegation(
    fsm, mock_state_repo, mock_voluntario_service, mock_faq_service
):
    ...
    mock_state_repo.save_or_update.assert_called_with(
        ANY, session_id, State.WAITING_NAME.name, ANY
    )

    # 2. Segunda mensagem: Envia o nome
    mock_state_repo.get_by_session_id.return_value = ConversationStateModel(
        session_id=session_id, state=State.WAITING_NAME.name, data={}
    )
    fsm.handle_message(session_id, "Carlos")
    mock_state_repo.save_or_update.assert_called_with(
        ANY, session_id, State.WAITING_KNOWLEDGE.name, {"nome": "Carlos"}
    )

    # ... e assim por diante para os outros passos ...
    mock_state_repo.get_by_session_id.return_value = ConversationStateModel(
        session_id=session_id,
        state=State.WAITING_KNOWLEDGE.name,
        data={"nome": "Carlos"},
    )
    ...
    mock_state_repo.get_by_session_id.return_value = ConversationStateModel(
        session_id=session_id,
        state=State.WAITING_LOCATION.name,
        data={
            "nome": "Carlos",
    ...
    mock_state_repo.get_by_session_id.return_value = ConversationStateModel(
        session_id=session_id,
        state=State.WAITING_HOBBIES.name,
        data={
            "nome": "Carlos",
            "conhecimentos": {"python": "avançado", "sql": "intermediário"},
            "local": "São Paulo/SP",
    ...
    mock_state_repo.save_or_update.assert_called_with(
        ANY, session_id, State.READY_TO_CHAT.name, ANY
    )

    # Delegação para o FAQ
    mock_state_repo.get_by_session_id.return_value = ConversationStateModel(
        session_id=session_id, state=State.READY_TO_CHAT.name, data={}
    )
...
def test_fsm_skips_onboarding_if_profile_exists(
    ...
    mock_state_repo.save_or_update.assert_called_with(
        ANY, session_id, State.READY_TO_CHAT.name, {}
    )
